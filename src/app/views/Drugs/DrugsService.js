...
var path = "/api/drug";
let DRUG_PATH = '/api/drug-patient'
...
export const addNew = adminUnit => {
    let url = "";
    if (adminUnit.patient) {
        url = ConstantList.API_ENPOINT + DRUG_PATH + '/save';
    } else {
        url = ConstantList.API_ENPOINT + path;
    }
    return axios.post(url, adminUnit);
};
